...
class Production_order extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/laporan/production_order/tampil',$data);
	...
	function detail($id_po)
	{
		...
		$this->load->view('admin/header',$data);  
		$this->load->view('admin/navbar',$data);
		$this->load->view('admin/laporan/production_order/detail',$data);
		$this->load->view('admin/footer');	
	}
	...
	$this->dompdf->stream("Laporan PO No. ".$data['production_order'][0]['no_po']." - ".$data['production_order'][0]['nama_po']." Tanggal ".date('d-m-Y', strtotime($data['production_order'][0]['tgl_order'])).".pdf", array("Attachment"=> 1));
}
